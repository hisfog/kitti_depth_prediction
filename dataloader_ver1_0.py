...
def preprocessing_transforms(mode):
    return transforms.Compose([ToTensor(mode = mode)])##implemented below

class kittiDataLoader(object):
    def __init__(self, args, mode):
        if mode == 'train':
            self.training_samples = DataLoadPreprocess(args)
            self.data = DataLoader(self.training_samples, args.batch_size, shuffle=True, pin_memory=True)
...
class DataLoadPreprocess(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.args.do_random_rotate is True:
            image = image_augment.random_rotate(image)
            depth_gt = image_augment.random_rotate(depth_gt, flag=Image.NEAREST)
        ...
        image = image_augment.random_crop(image, self.args.input_height, self.args.input_width)
        depth_gt = image_augment.random_crop(depth_gt, self.args.input_height, self.args.input_width)
